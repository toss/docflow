...
export async function loadPlugins(config: ResolvedConfig): Promise<Plugin[]> {
  ...
  for (const pluginConfig of config.plugins) {
    try {
      const pluginInstance = await pluginConfig.plugin(pluginConfig.options || {});

      if (!pluginInstance || typeof pluginInstance !== "object") {
        throw new Error(`Plugin factory for '${pluginConfig.name}' did not return a valid plugin object`);
      }

      const plugin = pluginInstance as Plugin;

      if (!plugin.hooks) {
        throw new Error(`Plugin '${pluginConfig.name}' must have hooks defined`);
      }
